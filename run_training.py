...
def main():
    ...
    try:
        ...
        update_status(f"✅ GPU detected: {gpu_name}. Loading pipeline...")

        # Add project root to sys.path for imports
        sys.path.insert(0, str(ROOT))
        from train.colab_training_pipeline import ColabTrainingPipeline

        update_status("Authenticating with Hugging Face Hub...")
        pipeline = ColabTrainingPipeline()
        pipeline.authenticate()

        update_status("Loading Qwen2.5-1.5B model (4-bit quantized, fp16)...")
        pipeline.load_model()

        update_status("Stage A: SFT Format Warmup — teaching the model output structure...")
        ...
        try:
            update_status("Generating intermediate training plots...")
        ...
        try:
            ...
            update_status(f"Final plots skipped (non-fatal): {plot_err}")

        if hf_repo:
            update_status(f"Uploading LoRA adapter weights to {hf_repo}...")
            pipeline.upload_to_hf(repo_id=hf_repo)
            update_status(f"✅ Done! Weights uploaded to {hf_repo}. Refresh 'Results & Evidence' tab.")
        else:
            update_status("✅ Training complete! Refresh 'Results & Evidence' tab to view plots.")
